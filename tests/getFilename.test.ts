...
await topLvlTest('getFilename', { concurrency: true }, async t => {
  await Promise.allSettled([
    ...
    t.test('should throw a TypeError for non-string inputs', () => {
      /* eslint-disable-next-line unicorn/no-null */ // @ts-expect-error -- testing
      assert.throws(() => getFilename(null), TypeError);

      // @ts-expect-error -- testing
      assert.throws(() => getFilename(), TypeError);

      /* eslint-disable-next-line @typescript-eslint/no-magic-numbers */// @ts-expect-error -- testing
