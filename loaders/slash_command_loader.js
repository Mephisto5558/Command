...
module.exports = async function slashCommandHandler(doneFn, cooldownsManager) {
  ...
  for (const subFolder of await getDirectories('./Commands')) {
    for (const file of await readdir(`./Commands/${subFolder}`, { withFileTypes: true })) {
      ...
      let command;
      try { command = commandFile.init(this.i18n, filePath, { logger: log, doneFn, cooldownsManager }); }
      catch (err) {
  ...
  this.on(Events.InteractionCreate, async interaction => {
    try { await require('../../Teufelsbot/Events').interactionCreate.call(interaction); }
