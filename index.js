...
function slashCommandsEqual(a, b) {
  ...
  if (a.name != b.name || a.description != b.description || a.type != b.type || a.autocomplete != b.autocomplete || a.defaultMemberPermissions?.bitfield != b.defaultMemberPermissions?.bitfield)
    return false;
  ...
  if (a instanceof Command.CommandOption && b instanceof Command.CommandOption) {
    ...
    for (const { name, nameLocalizations, value } of a.choices) {
      ...
      if (!choiceB || choiceB.value != value || !mapsEqual(choiceB.nameLocalizations, nameLocalizations)) return false;
    }
